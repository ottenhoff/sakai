...
public class JLDAPDirectoryProvider implements UserDirectoryProvider, LdapConnectionManagerConfig, ExternalUserSearchUDP, UsersShareEmailUDP, DisplayAdvisorUDP, AuthenticationIdUDP
{
	...
	public boolean authenticateUser(final String userLogin, final UserEdit edit, final String password)
	{
		...
		if ( !(isSearchableEid(userLogin)) ) {
			M_log.debug("authenticateUser(): user id in blacklist [userLogin = " + userLogin + "]");
			return false;
		}
		...
		try
		{
			...
			if ( M_log.isDebugEnabled() ) {
				M_log.debug("authenticateUser(): duke endUserDN: " + endUserDN + "::" + userLogin);
			}
		...
		catch (LDAPException e)
		{
			...
			if ( duke != null ) {
				if ( M_log.isDebugEnabled() ) {
					M_log.debug("authenticateUser(): closing duke conn");
				}
	...
	throws LDAPException {
		...
		if ( foundUserData == null ) {
			if ( M_log.isDebugEnabled() ) {
				M_log.debug("lookupUserEntryDN(): no directory entry found [eid = " + 
						eid + "]");
			}
			return null;
		}
